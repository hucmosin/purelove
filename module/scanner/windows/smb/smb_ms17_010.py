...
docs = '''
#==============================================================================
#title                  : MS17-010 VULNERABLE Check
#description
When Microsoft server message block (SMBv1) 1.0 server when handling certain requests,
there are multiple remote code execution vulnerability.
Successful use of these holes of an attacker can get in on the target system's capacity to enforce the code.
To take advantage of this vulnerability, in most cases,
unauthenticated attacker may send specially designed data to the target SMBv1 server.
This security update by dealing with these specially designed request correction SMBv1 way to repair the hole.
#author                 : mosin
#date                   : 20180503
#version                : 1.0
...
class PLScan(BGExploit):
    ...
    def __init__(self):
        ...
        self.info = {
            "name": "MS17-010",  # 该POC的名称
            "product": "MS17-010",  # 该POC所针对的应用名称,
...
This security update by dealing with these specially designed request correction SMBv1 way to repair the hole.
            ...
            "author": ["mosin"],  # 编写POC者
            "ref": [
                {self.ref.url: "https://github.com/RiskSense-Ops/MS17-010"},      # 引用的url
                {self.ref.bugfrom: "https://github.com/RiskSense-Ops/MS17-010"},  # 漏洞出处
            ],
            "type": self.type.rce,  # 漏洞类型
            "severity": self.severity.high,  # 漏洞等级
            "privileged": False,  # 是否需要登录
            "disclosure_date": "2017-05-10",  # 漏洞公开时间
            "create_date": "2018-05-03",  # POC 创建时间
        }
        ...
        self.register_result({
            ...
            #程序返回信息
            "description": " likely VULNERABLE to MS17-010!",
            "error": " "
        })
