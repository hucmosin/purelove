...
def main(bg_instance):
    ...
    try:
        ...
        print('result 无法被 json 序列化, 请不要将不可序列化的对象填写至 result 中').decode('utf-8')
        print(e)
        return
    ...
    if bg_instance.result.status:
        ...
        print(u'[详细说明]')
        print('\t{poc_desc}'.format(poc_desc=bg_instance.info.get('desc', '').strip())).decode('utf-8')

        print(u'[程序返回]')
        print('\t{poc_return}'.format(poc_return=bg_instance.result.get('description', '').strip())).decode('utf-8')

        print(u'[危害等级]')
        print('\t{poc_severity}'.format(poc_severity=bg_instance.info.get('severity', ''))).decode('utf-8')

        print(u'[漏洞类别]')
        print('\t{poc_type}'.format(poc_type=bg_instance.info.get('type', ''))).decode('utf-8')
