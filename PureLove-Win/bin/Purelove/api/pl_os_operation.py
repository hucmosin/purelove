...
def pl_show_all_poc_info(PL_PWD):
    PL_PWD = PL_PWD + "/bin/Purelove/logs/poc_name_path.pl"
...
'''
        ...
        print "   {Name:<55}{DisclosureDate:<20}{Rank:<20}{Descriptions:<40}".format(Name            = "Name",
                                                                                     ...
                                                                                     Rank           = "Rank",
                                                                                     Descriptions   = "Descriptions").decode('utf-8')
            ...
            for poc_name in lines:
                ...
                try:
                    ...
                    print "   {poc_name:<55}{date:<20}{severity:<20}{name:<40}".format(poc_name = pl_del_suffix(poc_name),
                                                                                       ...
                                                                                       name     = poc.info['name']).decode('utf-8')
                    print
                   # print "\t" + pl_del_suffix(poc_name) +pl_get_file_date(poc_name) + poc.info['severity'] + poc.info['name']
                except:
                    f.close()
        except:
            f.close()
...
def pl_find_poc_name(PL_PWD, PL_POC_NAME):
    PL_PWD = PL_PWD + "/bin/Purelove/logs/poc_name.pl"
    if PL_POC_NAME[-3:] == ".py":
        try:
            f = open(PL_PWD)
            lines = f.readlines()
